...
async function run() {
  const result = await pimms.workspaces.get({
    idOrSlug: "<value>",
  });
...
import { workspacesGet } from "pimms/funcs/workspacesGet.js";
...
async function run() {
  const res = await workspacesGet(pimms, {
    idOrSlug: "<value>",
  });
...
async function run() {
  const result = await pimms.workspaces.update("<value>");
...
```typescript
import { PimmsCore } from "pimms/core.js";
import { workspacesUpdate } from "pimms/funcs/workspacesUpdate.js";
...
async function run() {
  const res = await workspacesUpdate(pimms, "<value>");
